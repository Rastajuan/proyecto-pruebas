...
if(isset($_POST["correoContrasena"]) && $_SERVER["REQUEST_METHOD"] === 'POST') {
    $administrador = ManejadorAdministrador::obtenerAdministrador($_POST["correo"]);
    if (!empty($administrador)) {
        ...
        ManejadorAdministrador::modificarTokenContrasenaAdministrador($administrador["id"], $token); 
        $mail = new Mail(); 
        $mail->recordarContrasenaAdmin($administrador["mail"], $token); 
        $correcto = "Se ha enviado un correo a la dirección de correo que has introducido.";
    }else{
        $error = "El correo que ha introducido no pertenece a ningún administrador";
    }
}
...
<body>
    ...
    <div id="formulario" class="row d-flex justify-content-center align-content-center">
        <div class="row">
            <a href="formulario_login_jumac.php" class="linkVolver mb-5"><i class="bi bi-arrow-left-short"></i> Volver atrás</a>
        </div>
