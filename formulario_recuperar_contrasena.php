...
$usuario = null;
if(isset($_GET["token"])) {
    $usuario = ManejadorUsuario::obtenerUsuarioToken($_GET["token"]);
}

if (is_null($usuario) || !isset($_GET["token"])) {
...
if (isset($_POST["recuperarContrasena"])) {
    ...
    if ($nueva_contrasena == $nueva_contrasena_2) {
        if (ManejadorUsuario::recuperarContrasena($usuario["id"], $nueva_contrasena)) {
...
<body>
    ...
    <div id="formulario" class="row d-flex justify-content-center align-content-center">
        <div class="row">
            <a href="formulario_login.php" class="linkVolver mb-5"><i class="bi bi-arrow-left-short"></i> Iniciar sesión</a>
        </div>
